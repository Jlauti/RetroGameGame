...
echo "Camera FOV (radians):"
jq '.result[0].components["bevy_camera::projection::Projection"].Perspective.fov' "${after_file}"
echo "Fog density:"
jq '.result[0].components["bevy_pbr::fog::DistanceFog"].falloff.ExponentialSquared.density' "${after_file}"
echo
